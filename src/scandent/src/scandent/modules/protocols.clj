...
;; In Clojure however we have protocols – which are a powerful tool for
;; building APIs and shared abstractions in a way that solves the
;; expression problem. This makes it possible to create an API that can be
;; extended to deal with different implementations *without* having to either
;; modify the original implementation or create a wrapper.

(defprotocol PMatrixAdd
  "Protocol to support matrix addition on an matrices of same size"
  (matrix-add [m a]))
...
(extend-protocol PMatrixAdd
  clojure.lang.PersistentVector
    (matrix-add [m a]
      (.specialMatrixAdd m (.push a))))

;; Note that we are free to define how matrix addition is achieved for a specific
;; implementation in any way we like: perhaps even deferring the computation or
;; delegating it to a remote machine.

;; Finally, we can build a regular, easy to use Clojure API on top of the protocols.
;; Perhaps you might want to define vector addition, with something like the following:
